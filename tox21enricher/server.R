...
shinyServer(function(input, output, session) {
    ...
    observeEvent(input$refresh, {
        ...
        shinyjs::reset(id="submitted_chemicals")
        changePage(page="enrichment")
        # Re-enable includeChemsWithWarnings
        shinyjs::enable("includeChemsWithWarnings")
        ...
        if(mode == "annotation") {
            ...
            if (originalEnrichMode != "casrn") {
                lapply(names(reenrichResults), function(i) {
                    ...
                    resp <- NULL
                    trySVG <- tryCatch({
                        tmp_resp <- unlist(lapply(resultImagesSVG, function(smilesToSvg){
                            resp <- GET(url=paste0(API_PROTOCOL, API_ADDR, "/getStructureImages"), query=list(input=smilesToSvg))
                            content(resp)
                        }))
                        TRUE
                    }, error=function(cond){
