...
if(APP_DIR == "/"){
    print("Error: invalid value for APP_DIR.")
    stop()
}
if(IN_DIR == "/"){
    print("Error: invalid value for IN_DIR.")
    stop()
}
if(OUT_DIR == "/"){
    print("Error: invalid value for OUT_DIR.")
    stop()
}
# If not setting archive directory, then ask user if they would like to auto-delete archive files
if(nchar(ARCHIVE_DIR) < 1){
    archiveDirConfirm <- ""
    while(archiveDirConfirm != "y" & archiveDirConfirm != "N"){
        cat("Warning: ARCHIVE_DIR not set. Continuing will permanently delete previous request data without archival. Continue? [y/N]")
        archiveDirConfirm <- readLines("stdin", n=1)
    }
    if(archiveDirConfirm == "N"){
        stop()
    }
}
