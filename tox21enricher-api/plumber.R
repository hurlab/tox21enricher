...
queueResubmit <- function(res, req, mode="", enrichmentUUID="-1", annoSelectStr="", nodeCutoff=10, setNames){
    # Clean up queue
    if(DELETE_TIME != -1){
        future({
            queueCleanup()
...
#* @post /queue
queue <- function(res, req, mode="", enrichmentUUID="-1", annoSelectStr="", nodeCutoff=10, setNames){
    # Validate input
    ...
    if(nchar(annoSelectStr) < 1){
        res$status <- 400
        return("Error: no annotations selected.")
    }
...
createTransaction <- function(res, req, originalMode="", mode="", uuid="-1", annoSelectStr="", cutoff=10, input="", casrnBox="", originalNames="none", reenrich="", color="", timestampPosted="", reenrichFlag=FALSE, pvalueType="nominal"){
    # Validate input
