...
KAPPA <- NULL
if(!file.exists(paste0(APP_DIR, "/kappa.rds"))){
    print("Error: missing kappa.rds file in main application directory. Cannot perform enrichment analysis. Aborting...")
    stop()
}
KAPPA <- readRDS(paste0(APP_DIR, "/kappa.rds"))
...
performEnrichment <- function(enrichmentUUID="-1", annoSelectStr=fullAnnoClassStr, nodeCutoff=10, clustering=TRUE) {
    # Connect to db
    poolInput <- connQueue()
    ...
    create_david_chart_cluster(baseDirName, nodeCutoff, "ALL", "P", 0.05, "P", clustering)
    # zip result files
...
create_david_chart_cluster <- function(baseDirName="", topTermLimit=10, mode="ALL", sigColumnName="P", sigCutoff=0.05, valueColumnName="P", clustering=TRUE){
    # Get the corresponding directory names
...
# Perform enrichment analysis
perform_CASRN_enrichment_analysis <- function(CASRNRef, outputBaseDir, outfileBase, mappedCASRNsFromProcess, funCat2Selected, CASRN2funCatTerm, funCatTerm2CASRN, funCat2CASRNCount, funCat2termCount, funCatTerm2CASRNCount, pvalueThresholdToDisplay, similarityThreshold=0.50, initialGroupMembership, multipleLinkageThreshold, EASEThreshold, nodeCutoff=10, enrichmentUUID, clustering=TRUE){
    ...
    sortFunCatProcess <- lapply(sortFunCatProcess, function(x) x %>% slice(seq_len(nodeCutoff)))
...
kappa_cluster <- function(x, deg=NULL, useTerm=FALSE, cutoff=0.5, overlap=0.5, minSize=5, escore=3, outputBaseDir=NULL, outfileBase=NULL, sortedFunCatTerms=NULL, sigTerm2CASRNMatrix=NULL, sortedFunCatTermsCount=NULL, inputCASRNsCount=NULL, similarityThreshold=NULL, initialGroupMembership=NULL, multipleLinkageThreshold=NULL, EASEThreshold=NULL, term2Pvalue=NULL, term2Contents=NULL, enrichmentUUID=NULL, inputCASRNs=NULL){
    ...
    # # TODO: this pairwise comparison is horrifically inefficient
    # termpair2kappaOverThreshold <- mclapply(seq_len(nrow(termpair2kappaOverThreshold)), mc.cores=CORES+8, mc.silent=FALSE, function(term_row) {
    ...
    #     if (Aab != 1) {
    #         Kappa <- (Oab - Aab) / (1 - Aab) # don't round, it's slightly faster not to
    #         if (Kappa > similarityThreshold) {
    #             iTerm <- paste0(sortedFunCatTerms[term1])
    #             jTerm <- paste0(sortedFunCatTerms[term2])
    ...
    ######## Load pre-calculated kappa values list
    termpair2kappaOverThreshold <- KAPPA
    termpair2kappaOverThreshold <- termpair2kappaOverThreshold[names(termpair2kappaOverThreshold) %in% sortedFunCatTerms]
    termpair2kappaOverThreshold <- lapply(termpair2kappaOverThreshold, function(x) {
        tmp <- x[x %in% sortedFunCatTerms]
        if(length(tmp) < 1){
            return(NULL)
        }
        return(tmp)
    ...
    # Calculate enrichment scores for clusters
    ...
    writeToClusterFile <- apply(clusterDF, 1, function(clusterRow){
...
#* @post /queue
queue <- function(res, req, mode="", enrichmentUUID="-1", annoSelectStr=fullAnnoClassStr, nodeCutoff=10, setNames, clustering=TRUE){
    # Validate input
    ...
    future({
        ...
        if(mode == "annotation") {
            ...
            status_code <- performEnrichment(enrichmentUUID=enrichmentUUID, annoSelectStr=annoSelectStr, nodeCutoff=nodeCutoff, clustering=clustering)
        } 
...
submit <- function(res, req, mode="", input="", annotations="", cutoff=10, tanimoto=0.5) {
    ...
    # } else { #TODO: not sure if this bit is necessary
